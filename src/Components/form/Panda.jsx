...
function PandaKeyboard() {
  ...
  useEffect(() => {
    ...
    const leftear = container.querySelector(".leftear");
    const rightear = container.querySelector(".rightear");

    const handleMouseMove = (e) => {
      const rect = container.getBoundingClientRect();
      const xValue = (e.clientX - rect.left - rect.width / 2) / 20;
      const yValue = (e.clientY - rect.top - rect.height / 2) / 20;
  ...
  return (
    <div className="container" ref={containerRef}>
      ...
      {["C4", "D4", "E4", "F4", "G4", "A4", "B4", "C5"].map((note) => (
        <div key={note} className="key" data-note={note}></div>
      ))}
